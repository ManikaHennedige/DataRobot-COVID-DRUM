...
API_URL = 'https://app.imda-tal-ent.sg.datarobot.com/predApi/v1.0/deployments/{deployment_id}/predictionsUnstructured'    # noqa
API_KEY = ''
...
def make_datarobot_deployment_unstructured_predictions(data, deployment_id, mimetype, charset):
    ...
    See docs for details:
         https://app.imda-tal-ent.sg.datarobot.com/docs/predictions/api/dr-predapi.html
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        exit(1)
    filename = sys.argv[1]
    mimetype = sys.argv[2] if len(sys.argv) >= 3 else 'multipart/x-zip' # 'text/plain'
    charset = sys.argv[3] if len(sys.argv) == 4 else 'application/zip'  # 'UTF-8'
