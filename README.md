...
1. 'app' folder
    ...
    This folder contains files containing CLI applications demonstrating DataRobot/DRUM functionality.
...
This folder contains two CLI applications. The first script is used to generate data, while the second script then uses this data to perform predictions on models hosted on the DataRobot/DRUM API.
...
1. predict.py
    ...
    This script is adapted from the official DR script, and is usable across a wider variety of use cases. It allows real-time predictions to be made on the online DataRobot instance, or DRUM platform.
...
The 'load.py' file contains scripts for a CLI application for users to generate a csv file for prediction. Before running the script, ensure that the following conditions are met:
